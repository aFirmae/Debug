    ...
    def upload_to_mysql(file_path, table_name, columns_to_upload):
        ...
        try:
            if file_path.endswith('.csv'):
                data = pd.read_csv(file_path, usecols=columns_to_upload, na_values=None, encoding='ISO-8859-1')
            elif file_path.endswith('.xlsx') or file_path.endswith('xls'):
                data = pd.read_excel(file_path, usecols=[col.strip() for col in columns_to_upload], na_values=None)
            else:
